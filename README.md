...
### AI Providers
- Managed AI turns are served by OpenAI (the controller pins `CODEX_MODEL_PROVIDER=openai`; the model comes from `MANAGED_AI_MODEL_ID`, default `gpt-6-luna`). Bring-your-own ChatGPT logins and OpenAI API keys default to `gpt-5.6-sol` instead, since the user pays for those.
