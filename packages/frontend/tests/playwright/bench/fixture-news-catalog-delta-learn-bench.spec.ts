...
test.describe("Bench: /learn improves deterministic paginated navigation (opt-in)", () => {
  ...
  test.beforeAll(async () => {
    fixtureSite = await startBenchFixtureSite("fixture-news-catalog-delta");
  });
  ...
  test.afterEach(async ({ page }) => {
    await resetRuntimeUserState(page, { source: "bench:fixture-news-catalog-delta:cleanup" }).catch(() => {});
  });

  test("before/after /learn (fixture catalog -> delta)", async ({ page }) => {
    ...
    const benchDir = "bench/fixture-news-catalog-delta";
    await writeWorkspaceFile(page, `${benchDir}/README.md`, "# Fixture news catalog-delta /learn benchmark\n", {
    ...
    const attemptPrompt = (label: string, attemptNumber: number) =>
      [
        ...
        "ARTICLE_URL: <url>",
        "ARTICLE_TOKEN: <token>",
        "PAGE_ADVANCE_CUE: exact pagination control text used, or -",
    ...
    await recycleHostedRuntimeForBench(page, projectId, "bench:fixture-news-catalog-delta:post-reset");
    ...
    await updateLearnBenchSummary(page, {
      projectId,
      benchKey: "fixture-news-catalog-delta",
      benchDir,
      ...
      attempts: attempts.map((attempt) => ({
        ...
        pageAdvanceCue: attempt.pageAdvanceCue,
    ...
    const lines = [
      "# Fixture news catalog-delta /learn benchmark",
      "",
