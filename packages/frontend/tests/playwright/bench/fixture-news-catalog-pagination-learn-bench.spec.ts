...
import { runCatalogPaginationAttempt, type CatalogPaginationAttemptResult } from "./benchTasks.js";
import { updateLearnBenchSummary } from "./learnBenchSummary.js";

test.describe("Bench: /learn improves deterministic catalog pagination (opt-in)", () => {
  ...
  test.beforeAll(async () => {
    fixtureSite = await startBenchFixtureSite("fixture-news-catalog-pagination");
  });
  ...
  test.afterEach(async ({ page }) => {
    await resetRuntimeUserState(page, { source: "bench:fixture-news-catalog-pagination:cleanup" }).catch(() => {});
  });

  test("before/after /learn (fixture catalog pagination -> page 2)", async ({ page }) => {
    ...
    const expectedPathname = "/catalog";
    const expectedQuery = "?page=2";

    const projectId = await prepareStudio(page);
    if (!projectId) throw new Error("Project id missing for fixture-news-catalog-pagination bench.");

    await page.getByTestId("sidebar-nav-chat").click();
    await clearRuntimePreference(page, { projectId, source: "bench:fixture-news-catalog-pagination" }).catch(() => {});
    ...
    const benchDir = "bench/fixture-news-catalog-pagination";
    await writeWorkspaceFile(page, `${benchDir}/README.md`, "# Fixture news catalog-pagination /learn benchmark\n", {
    ...
    const attemptPrompt = (label: string, attemptNumber: number) =>
      [
        ...
        "Stop when page 2 is visible and the Delta listing is visible on that page.",
        "",
        "Do not open the Delta article. Do not use curl/fetch; use only the browser session.",
        "",
        "In your final reply, include lines exactly like:",
        "PAGE_TITLE: <title>",
        "PAGE_URL: <url>",
        "DELTA_LISTING_VISIBLE: yes",
      ].join("\n");

    const attempts: CatalogPaginationAttemptResult[] = [];
    let learnOutcome: {
      command: string;
      signal: string | null;
      error: string | null;
    ...
    for (let preIndex = 1; preIndex <= preAttempts; preIndex += 1) {
      ...
      const attemptResult = await runCatalogPaginationAttempt(
        page,
        attemptIndex,
        attemptPrompt(`pre ${preIndex}/${preAttempts}`, attemptIndex),
        budgetSec,
        {
          expectedOrigin,
          expectedPathname,
      ...
      attempts.push(enriched as CatalogPaginationAttemptResult);
    ...
    await recycleHostedRuntimeForBench(page, projectId, "bench:fixture-news-catalog-pagination:post-reset");
    ...
    for (let postIndex = 1; postIndex <= postAttempts; postIndex += 1) {
      ...
      const attemptResult = await runCatalogPaginationAttempt(
        page,
        attemptIndex,
        attemptPrompt(`post ${postIndex}/${postAttempts}`, attemptIndex),
        budgetSec,
        {
          expectedOrigin,
          expectedPathname,
    ...
    await updateLearnBenchSummary(page, {
      projectId,
      benchKey: "fixture-news-catalog-pagination",
      benchDir,
    ...
    const lines = [
      "# Fixture news catalog-pagination /learn benchmark",
      "",
      `Fixture: ${fixtureBaseUrl}`,
      `Expected: ${expectedOrigin}${expectedPathname}${expectedQuery}`,
    ...
    const aiQuality =
      (await writeLearnBenchDiagnostics(page, {
        projectId,
        benchDir,
