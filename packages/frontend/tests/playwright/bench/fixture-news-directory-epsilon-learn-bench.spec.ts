...
test.describe("Bench: /learn improves deterministic filter-table navigation (opt-in)", () => {
  ...
  test.beforeAll(async () => {
    fixtureSite = await startBenchFixtureSite("fixture-news-directory-epsilon");
  });
  ...
  test.afterEach(async ({ page }) => {
    await resetRuntimeUserState(page, { source: "bench:fixture-news-directory-epsilon:cleanup" }).catch(() => {});
  });

  test("before/after /learn (fixture directory -> epsilon)", async ({ page }) => {
    ...
    const expectedPathname = "/article/epsilon";
    const expectedToken = "EPSILON-TOKEN-6f10bd";
    ...
    const benchDir = "bench/fixture-news-directory-epsilon";
    await writeWorkspaceFile(page, `${benchDir}/README.md`, "# Fixture news directory-epsilon /learn benchmark\n", {
    ...
    const attemptPrompt = (label: string, attemptNumber: number) =>
      [
        ...
        `Navigate to ${fixtureBaseUrl}/directory`,
        'If a cookie dialog appears, click "I agree".',
        'Use the filter UI to choose team "green" and apply the filter.',
        "Open the Epsilon account from the filtered table.",
    ...
    await recycleHostedRuntimeForBench(page, projectId, "bench:fixture-news-directory-epsilon:post-reset");
    ...
    await updateLearnBenchSummary(page, {
      projectId,
      benchKey: "fixture-news-directory-epsilon",
      benchDir,
    ...
    const lines = [
      "# Fixture news directory-epsilon /learn benchmark",
      "",
