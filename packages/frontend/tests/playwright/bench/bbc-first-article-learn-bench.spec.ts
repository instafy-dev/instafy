...
function looksLikeValidBbcArticle(title: string | null, url: string | null): boolean {
  if (!title || !url) return false;
  if (title.includes("<") || url.includes("<")) return false;
  if (!/^https?:\/\//i.test(url)) return false;
  if (!/bbc\.com\//i.test(url)) return false;
  try {
    const parsed = new URL(url);
    if (parsed.pathname === "/" || parsed.pathname.trim() === "") return false;
    // Typical BBC story urls include /news/ or /articles/; accept other non-root paths too.
  } catch {
    return false;
  }
  return true;
...
async function runBbcAttempt(
  page: Page,
  attempt: number,
  prompt: string,
  budgetSec: number,
): Promise<BbcAttemptResult> {
  ...
  while (Date.now() < deadline) {
    ...
    if (count > baselineAssistantCount) {
      ...
      if (currentText && currentText !== lastSeenText) {
        ...
        if (looksLikeValidBbcArticle(articleTitle, articleUrl)) {
          break;
        }
      }
    }
  ...
  const assistantMatch = (content: string) => {
    ...
    return looksLikeValidBbcArticle(parsed.title, parsed.url);
  };
  ...
  return {
    ...
    articleTitle,
    articleUrl,
    success: looksLikeValidBbcArticle(articleTitle, articleUrl),
    browserVisible,
    assistantTurns,
    assistantSummary,
    runId: metrics.runId,
...
test.describe("Bench: /learn improves BBC first-article open (opt-in)", () => {
  ...
  test.afterEach(async ({ page }) => {
    await resetRuntimeUserState(page, { source: "bench:bbc:cleanup" }).catch(() => {});
  });

  test("before/after /learn (BBC first article)", async ({ page }) => {
    page.setDefaultTimeout(60_000);

    const budgetSec = Number(process.env.PLAYWRIGHT_BENCH_BBC_BUDGET_SECS ?? "120");
    const iterationsRaw = Number(process.env.PLAYWRIGHT_BENCH_BBC_ITERATIONS ?? "2");
    ...
    const benchDir = "bench/bbc-first-article";
    await writeWorkspaceFile(page, `${benchDir}/README.md`, "# BBC first-article /learn benchmark\n", {
    ...
    const attemptPrompt = (attemptNumber: number) =>
      [
        ...
        "",
        "Open a live browser session (visible inline) and navigate to https://www.bbc.com/.",
        "If a cookie/consent dialog appears, accept it.",
        "Open the first visible BBC news story/article on the homepage (avoid Register/Sign in/navigation links).",
    ...
    const attempts: BbcAttemptResult[] = [];
    const learnOutcomes: {
      iteration: number;
      command: string;
      signal: string | null;
    ...
    for (let iteration = 1; iteration <= iterations; iteration += 1) {
      ...
      const attemptResult = await runBbcAttempt(page, iteration, attemptPrompt(iteration), budgetSec);
    ...
    reportLines.push("# BBC first-article /learn benchmark");
    reportLines.push("");
