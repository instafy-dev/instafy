...
import { runFixtureNewsAttempt, type FixtureNewsAttemptResult } from "./benchTasks.js";
...
test.describe("Bench: /learn improves deterministic fixture-site navigation (opt-in)", () => {
  ...
  test.beforeAll(async () => {
    fixtureSite = await startBenchFixtureSite("fixture-news-first-article");
  });
  ...
  test.afterEach(async ({ page }) => {
    await resetRuntimeUserState(page, { source: "bench:fixture-news:cleanup" }).catch(() => {});
  });

  test("before/after /learn (fixture first article)", async ({ page }) => {
    page.setDefaultTimeout(60_000);

    const learnCommand = (process.env.PLAYWRIGHT_BENCH_LEARN_COMMAND ?? "/learn 6").trim() || "/learn 6";
    const budgetSec = Number(process.env.PLAYWRIGHT_BENCH_FIXTURE_NEWS_BUDGET_SECS ?? "120");
    ...
    const expectedPathname = "/article/alpha";
    ...
    const benchDir = "bench/fixture-news-first-article";
    await writeWorkspaceFile(page, `${benchDir}/README.md`, "# Fixture news first-article /learn benchmark\n", {
    ...
    const attempts: FixtureNewsAttemptResult[] = [];
    let learnOutcome: {
      command: string;
      signal: string | null;
      error: string | null;
    ...
    for (let preIndex = 1; preIndex <= preAttempts; preIndex += 1) {
      ...
      const attemptResult = await runFixtureNewsAttempt(page, attemptIndex, attemptPrompt(`pre ${preIndex}/${preAttempts}`, attemptIndex), budgetSec, {
        expectedOrigin,
        expectedPathname,
      });
      ...
      attempts.push(enriched as FixtureNewsAttemptResult);
    ...
    await recycleHostedRuntimeForBench(page, projectId, "bench:fixture-news-first-article:post-reset");
    await openNewConversation(page);
    ...
    for (let postIndex = 1; postIndex <= postAttempts; postIndex += 1) {
      ...
      const attemptResult = await runFixtureNewsAttempt(page, attemptIndex, attemptPrompt(`post ${postIndex}/${postAttempts}`, attemptIndex), budgetSec, {
        expectedOrigin,
        expectedPathname,
      });
      const routingSnapshot = await collectLearnRoutingSnapshot(page, projectId).catch(() => null);
    ...
    await updateLearnBenchSummary(page, {
      projectId,
      benchKey: "fixture-news-first-article",
      benchDir,
    ...
    reportLines.push("# Fixture news first-article /learn benchmark");
    reportLines.push("");
