  ...
  return (
    <div className="flex shrink-0 items-center gap-2 text-xs" data-testid="browser-human-input-controls" data-browser-session-safe-zone="true">
      {state.active && props.humanControlConfirmed ? (
        ...
        <span role="status" className="text-slate-500 dark:text-slate-400">Stopping AI…</span>
      ) : props.canTakeOver ? (
      ...
      {state.active ? <span className="sr-only" role="status">{props.humanControlConfirmed ? manualGuidance : "Waiting for agent control to stop. Input remains locked."}</span> : null}
