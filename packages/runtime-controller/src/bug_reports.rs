...
use crate::errors::{bad_request, forbidden, internal_error, not_found, unauthorized, ApiError};
...
async fn list_bug_reports(
        ...
        connection
            .query(
                "select br.*,
                  ...
                  limit $1",
                &[&limit, &status, &reporter_email, &project_id, &before_created_at, &search],
            )
            .await
    } else {
        let user_id = context
            ...
            .query(
                "select br.*,
                  ...
                  where br.user_id = $1
                    ...
                    and ($4::text is null or lower(coalesce(br.reporter_email, '')) = lower($4))
                    and ($5::uuid is null or br.project_id = $5)
                    and ($6::timestamptz is null or br.created_at < $6)
                    and (
                      $7::text is null
                      or br.message ilike ('%' || $7 || '%')
                      or coalesce(br.details, '') ilike ('%' || $7 || '%')
                      or coalesce(br.reporter_email, '') ilike ('%' || $7 || '%')
                      or coalesce(br.project_id::text, '') ilike ('%' || $7 || '%')
                      or coalesce(br.conversation_id::text, '') ilike ('%' || $7 || '%')
                    )
                  order by br.created_at desc
                  limit $2",
                &[
    ...
    let reports = rows.into_iter().map(map_bug_report_summary_row).collect();
    Ok(Json(BugReportListResponse { reports }))
}
...
async fn get_bug_report(
    ...
    if !can_view_all {
        ...
        if owner_user_id != Some(viewer) {
            return Err(forbidden("bug report not accessible"));
        }
    }
    ...
    Ok(Json(BugReportDetailResponse {
