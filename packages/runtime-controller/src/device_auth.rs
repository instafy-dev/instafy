...
async fn start_device_auth(
    ...
    match provider.as_str() {
        ...
        // Google no longer permits this login for Gemini, so the flow is gone
        // rather than merely unconfigured. Named explicitly so an old client
        // gets the reason instead of "unknown device auth provider".
        "gemini" | "google" | "google-ai" | "google_gemini" | "google-gemini" => Err(bad_request(
