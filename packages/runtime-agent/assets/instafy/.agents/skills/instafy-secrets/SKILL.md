---
name: instafy-secrets
description: Safe secret handling policy using project secret manager actions.
---

# Secrets (project secret manager) — agent guidance
...
- Secrets live in **Studio → Settings → Project → Secrets**.
...
- For auth/integration tasks, inspect project secret metadata on demand before requesting a new secret:
  - `instafy secrets list --project <project-id> --json`
