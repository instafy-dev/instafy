#!/usr/bin/env node
// Pure authorization of a hosted web release tag. The tag is the ledger: it
// names the exact protected-main commit whose hosted web frontend is built and
...
export const TAG_PATTERN = /^web-v([0-9a-f]{12})(?:-r([2-9]|[1-9][0-9]))?$/u;
...
export function verifyReleaseTag(input) {
  ...
  if (input.repository !== REPOSITORY) {
    fail(`Hosted web releases publish only from ${REPOSITORY}`);
  }
  ...
  if (input.actor !== RELEASE_ACTOR || input.triggeringActor !== RELEASE_ACTOR) {
    fail(`Hosted web releases may only be started by ${RELEASE_ACTOR}`);
  }
  ...
  if (eventName === "push") {
    if (input.pusher !== RELEASE_ACTOR) {
      fail(`Hosted web release tags may only be pushed by ${RELEASE_ACTOR}`);
    }
    ...
    if (input.ref !== "refs/heads/main") {
      fail("A hosted web dry run dispatch must run main's workflow (--ref main)");
    }
  ...
  if (!match) {
    fail("The hosted web release tag must match web-v<first 12 hex of the commit>, optionally with -r<2-99>");
  }
