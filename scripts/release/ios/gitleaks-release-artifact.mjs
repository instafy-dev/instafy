#!/usr/bin/env node
// Pinned Gitleaks gate for the final signed IPA (scanned as a .zip alias) using
// the public boundary config plus release-artifact-only allowlists.
  ...
  try {
    const scan = run(
      ...
      [
        ...
        "--report-path",
        reportPath,
        // Upper bound, not a budget: a healthy IPA scan finishes far sooner.
        "--timeout=290",
      ],
