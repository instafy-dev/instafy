...
function reportDockerFailure(log, stage, image, result, thrown) {
  ...
  if (typeof stderr === "string" && Buffer.byteLength(stderr, "utf8") <= 32_768) {
    for (const [hint, pattern] of [
      ...
      ["tls", /x509:|tls(?:\s+|:\s*)handshake|certificate verify failed/i],
      ["tls-certificate", /x509:|certificate verify failed/i],
      ["tls-handshake-timeout", /tls handshake timeout/i],
      ["tls-handshake-rejected", /tls:\s*handshake failure/i],
