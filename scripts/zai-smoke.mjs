...
const DEFAULT_ENV_FILE = ".env.zai";
...
async function postJson(url, apiKey, body, timeoutMs) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Expected JSON but got non-JSON response: ${text.trim().slice(0, 200)}`
      );
    }
  } finally {
...
function printUsage() {
  console.log(`z.ai smoke test (safe, no local file scanning)

Usage:
  node scripts/zai-smoke.mjs [--env-file .env.zai] [--base-url <url>] [--model <model>]
                            [--endpoint auto|responses|chat] [--timeout-ms 20000] [--debug]

Env (preferred):
  ZAI_API_KEY           Required (or set in --env-file)
  ZAI_BASE_URL          Optional (default: ${DEFAULT_BASE_URL})
  ZAI_MODEL             Optional (default: ${DEFAULT_MODEL})
`);
}
...
async function main() {
  ...
  const apiKey = String(process.env.ZAI_API_KEY || fileEnv.ZAI_API_KEY || "").trim();
  if (!apiKey) {
    throw new Error(
      "Missing ZAI_API_KEY. Set it in the environment or add it to .env.zai."
    );
  }
  ...
  const baseUrl = normalizeBaseUrl(
    readOption(args, "--base-url") ||
      ...
      fileEnv.ZAI_BASE_URL ||
      DEFAULT_BASE_URL
  );

  const model = String(
    readOption(args, "--model") || process.env.ZAI_MODEL || fileEnv.ZAI_MODEL || DEFAULT_MODEL
  ).trim();
  if (!model) {
    throw new Error("Missing model. Set --model or ZAI_MODEL.");
  }

  const endpointMode = String(
    readOption(args, "--endpoint") || process.env.ZAI_ENDPOINT || "auto"
  )
    .trim()
    .toLowerCase();

  const timeoutMsRaw = readOption(args, "--timeout-ms") || process.env.ZAI_TIMEOUT_MS;
  const timeoutMs = Number(timeoutMsRaw || DEFAULT_TIMEOUT_MS);
  const debug = readFlag(args, "--debug");

  if (!baseUrl) {
    throw new Error("Missing base URL. Set --base-url or ZAI_BASE_URL.");
  }
  ...
  if (endpointMode === "responses" || endpointMode === "auto") {
    try {
      ...
      console.warn(`[zai-smoke] /v1/responses failed; falling back to /v1/chat/completions (${message})`);
    }
  }
  ...
  console.log(`[zai-smoke] endpoint: ${result.url}`);
  console.log(`[zai-smoke] mode: ${result.mode}`);
  console.log(`[zai-smoke] model: ${model}`);
  console.log(`[zai-smoke] output: ${JSON.stringify(trimmed)}`);
  console.log(`[zai-smoke] status: ${ok ? "PASS" : "WARN"} (${elapsedMs}ms)`);
  if (!ok) {
    console.log(`[zai-smoke] expected: "OK" (exact).`);
  }

  if (debug) {
    console.log("[zai-smoke] raw response JSON:");
...
main().catch((error) => {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`[zai-smoke] FAIL: ${message}`);
  process.exit(1);
});
